...
if __name__ == "__main__":
    ...
    def generate(self, document: Document, output_path: str, 
        ...
        if theme not in self.available_themes:
            available = self.get_available_themes()
            if available:
                theme = available[0]  # 回退到第一个可用主题
                print(f"警告: 主题不存在，使用默认主题: {theme}")
            else:
                raise ValueError(f"没有可用的CSS主题文件，请在 {self.static_css_dir} 中添加CSS文件")
    ...
    def _copy_theme_css(self, theme: str, output_dir: Path) -> str:
        ...
        try:
            shutil.copy2(source_css, target_css)
            print(f"CSS文件已复制: {source_css} -> {target_css}")
            return f"{theme}.css"
        except Exception as e:
            print(f"警告: 无法复制CSS文件 {source_css}: {e}")
            # 尝试回退到默认主题
            if theme != "RemStyle" and "RemStyle" in self.available_themes:
                return self._copy_theme_css("RemStyle", output_dir)
            raise
